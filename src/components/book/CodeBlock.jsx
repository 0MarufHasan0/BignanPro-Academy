          ...
          {lines.map((line, i) => {
            ...
            return (
              <div
                key={i}
                onClick={() =>
                  hasExplanation && setActiveLine(isActive ? null : lineNum)
                }
                className={`h-6 px-1.5 -mx-1.5 rounded transition-colors group relative ${
                  hasExplanation
                    ? "cursor-pointer hover:bg-primary/10 border-l-2 border-dashed border-primary/45"
                    : ""
                } ${
                  isActive
                    ? "bg-primary/20 border-l-2 border-solid border-primary"
                    : ""
                }`}
              >
                <span>{line || " "}</span>
