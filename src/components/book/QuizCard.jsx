  ...
  return (
    <div className="my-6 border border-border/80 rounded-2xl bg-card text-card-foreground shadow-sm hover:shadow-md transition-shadow p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        {submitted && (
          <span
            className={`flex items-center gap-1 text-xs font-bold ${isCorrect ? "text-emerald-500" : "text-destructive"}`}
          >
            {isCorrect ? (
              <>
        ...
        {options.map((opt, idx) => {
          ...
          if (submitted) {
            if (isAnswer) {
              ...
              optionStyle =
                "border-destructive bg-destructive/10 text-destructive-foreground ring-1 ring-destructive/30";
            } else {
