...
import { Brain, HelpCircle, Lightbulb, UserCheck, ShieldAlert, Bug, ChevronDown } from "lucide-react";
import { AIMentorNote } from "@/types";
...
export function AIMentor({ notes }: AIMentorProps) {
  const { localize, language: currentLang } = useLanguage();
  const [openSection, setOpenSection] = useState<string | null>("whyExists");
  ...
  const sectionsList = [
    {
      ...
      title: { bn: "ধারণাটি কেন প্রয়োজন? (Why it exists)", en: "Why this concept exists" },
      content: notes.whyExists,
    },
    {
      id: "whyWorks",
      icon: <Lightbulb className="w-4 h-4 text-amber-500" />,
      title: { bn: "কোডটি কীভাবে কাজ করছে? (Why it works)", en: "Why this code works" },
      content: notes.whyWorks,
    },
    {
      id: "professionalThinking",
      icon: <UserCheck className="w-4 h-4 text-emerald-500" />,
      title: { bn: "প্রফেশনালরা যেভাবে চিন্তা করেন (Pro Mindset)", en: "How professionals think" },
      content: notes.professionalThinking,
    },
    {
      id: "mistakes",
      icon: <ShieldAlert className="w-4 h-4 text-rose-500" />,
      title: { bn: "নতুনদের সাধারণ ভুলসমূহ (Common Mistakes)", en: "Common beginner mistakes" },
      content: notes.mistakes,
    },
    {
      id: "debugging",
      icon: <Bug className="w-4 h-4 text-cyan-500" />,
      title: { bn: "ডিবাগিং ও পারফরম্যান্স (Debugging & Perf)", en: "Debugging & Performance" },
      ...
      <div className="flex items-center gap-2.5 pb-4 mb-4 border-b border-primary/10">
        ...
        <div>
          <h3 className="text-md font-bold tracking-tight">
            {currentLang === "bn" ? "🤖 এআই মেন্টর গাইডেন্স" : "🤖 AI Mentor Insights"}
          </h3>
          <p className="text-[11px] text-muted-foreground">
            {currentLang === "bn" ? "ধারণা গভীর করতে প্রফেশনাল গাইডেন্স" : "Deepen understanding with industry insights"}
          </p>
        </div>
      </div>
