...
export function Navbar({ toggleSidebar, showSidebarButton = false }: NavbarProps) {
  ...
  const themes: { id: ThemeType; label: { bn: string; en: string }; icon: React.ReactNode }[] = [
    { id: "light", label: { bn: "লাইট মোড", en: "Light Mode" }, icon: <Sun className="w-4 h-4" /> },
    { id: "dark", label: { bn: "ডার্ক মোড", en: "Dark Mode" }, icon: <Moon className="w-4 h-4" /> },
    {
      id: "reading",
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-md transition-colors duration-300">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-4">
          {/* Quick links */}
          <Link href="/book" className="text-sm font-semibold hover:text-primary transition-colors px-2 py-1">
            {language === "bn" ? "বইয়ের সূচী" : "Book Index"}
          </Link>
          <Link href="/notes" className="text-sm font-semibold hover:text-primary transition-colors px-2 py-1">
          ...
          <div
            ...
            title={language === "bn" ? "আপনার শেখার স্ট্রিক" : "Your learning streak"}
          >
            <Flame className="w-4 h-4 fill-amber-500 text-amber-500 animate-pulse" />
            <span>{streak} {language === "bn" ? "দিন" : "days"}</span>
          </div>
          ...
          <button
            ...
            title={language === "bn" ? "অক্ষরের সাইজ বড় করুন" : "Toggle Large Font Size"}
          ...
          <button
            ...
            title={language === "bn" ? "English এ পরিবর্তন করুন" : "Switch to বাংলা"}
            id="lang-toggle-btn"
          >
          ...
          <div className="relative">
            ...
            >
              ...
              {theme === "reading" && <BookOpen className="w-4 h-4 text-amber-600" />}
              {theme === "high-contrast" && <Eye className="w-4 h-4 text-emerald-500" />}
            </button>
                  ...
                  {themes.map((t) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-left text-xs font-semibold hover:bg-secondary/70 transition-colors ${
                        theme === t.id ? "text-primary bg-primary/5" : "text-foreground"
                      }`}
                      id={`theme-opt-${t.id}`}
                    >
                      {t.icon}
              ...
              {themes.map((t) => (
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-2.5 rounded-lg border text-[10px] font-semibold transition-all ${
                    theme === t.id ? "border-primary bg-primary/5 text-primary" : "border-border/60 hover:bg-secondary/40"
                  }`}
                >
                  {t.icon}
          ...
          <div className="flex items-center justify-between border-t border-border/40 pt-4">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-xs font-semibold ${
                largeFont ? "bg-primary/10 border-primary/30 text-primary" : "border-border/60"
              }`}
            >
              <Type className="w-4 h-4" />
