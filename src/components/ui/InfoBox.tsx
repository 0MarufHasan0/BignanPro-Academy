...
export function InfoBox({ variant = "info", title, children }: InfoBoxProps) {
  ...
  return (
    <div className={`my-6 flex gap-4 p-4 border rounded-xl ${current.bg} transition-all duration-200 shadow-sm`}>
      {current.icon}
      <div className="flex-1 space-y-1">
        ...
        <div className="text-sm leading-relaxed font-normal opacity-95">{children}</div>
      </div>
    </div>
  );
}
