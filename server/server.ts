...
const MONGODB_URI = process.env.MONGODB_URI || "mongodb://localhost:27017/bignanpro-academy";
...
app.get("/api/users/:email/progress", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
app.post("/api/users/complete-chapter", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// 4. Toggle Bookmark
app.post("/api/users/toggle-bookmark", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
app.post("/api/auth/register", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json({ message: "Registration successful", email: user.email, username: user.username });
  } catch (err) {
    next(err);
  }
});
...
app.get("/api/notes", async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const query: any = { email: (email as string).toLowerCase() };
